...
parser.add_argument('--dataset', type=str, default="logit_z", choices=[
                    "celebaz_feature", "logit", "logit_z", "logit_2z",  "house_price", "landcover"])
...
parser.add_argument('--irm_type', default="irmv1", type=str,
                    choices=["erm", "infer_irmv1", "infer_irmv1_multi_class", "lff", "eiil", "dro","drof"])
...
parser.add_argument('--cons_train', type=str, default="0.999_0.7") #c
...
assert flags.envs_num_test + \
    flags.envs_num_train == len(flags.cons_ratio.split("_"))
...
for restart in range(flags.n_restarts):
    ...
    dp, mlp, test_batch_num, train_batch_num, val_batch_num, test_batch_fetcher, mean_nll, mean_accuracy, eval_acc = init_dataset(
        flags)
    ...
    if flags.opt == "adam":
        ...
        optimizer = optim.SGD(params,
                              momentum=0.9, lr=flags.lr)  # +[algo.eta,]
    if flags.dataset == "house_price":
        meta_acc_test = MetaAcc(env=dp.envs_num_test,
                                acc_measure=mean_accuracy, acc_type="test")
    else:
        meta_acc_test = MetaAcc(env=flags.envs_num_test,
                                acc_measure=mean_accuracy, acc_type="test")

    pretty_print_ly(['step', 'train penalty', 'train acc'] +
                    meta_acc_test.acc_fields)
    best_results = {'val_acc': 0.0, 'epoch': -
                    1, 'train_acc': 0.0, 'test_acc': 0.0}
    ...
    for step in range(flags.steps):
        ...
        for batch in range(train_batch_num):
            ...
            if flags.irm_type == "erm":
                penalty_weight = 0
            train_penalty = torch.max(torch.tensor(
                0.0).cuda(), train_penalty.cuda())
            loss += penalty_weight * train_penalty
            if penalty_weight > 1.0:
                loss /= (1. + penalty_weight)

            optimizer.zero_grad()
            (loss).backward(retain_graph=True,inputs=list(params))
            optimizer.step()
            losses.append(loss)
            wandb.log({"loss": loss})
        ...
        if step % flags.print_every == 0:
            ...
            if val_batch_num:
                ...
                for ii in range(val_batch_num):
                    ...
                    val_acc, val_minacc, val_majacc = eval_acc(
                        val_logits, val_y, val_c)
    ...
    final_test_accs.append(
        meta_acc_test_res["test_acc"].detach().cpu().numpy())
