...
class DROF:
    ...
    def __init__(self, flags, dp):
        eta_init = torch.tensor([0.0], requires_grad=True).cuda()
        self.eta = torch.nn.parameter.Parameter(eta_init)
        print("eta_init", eta_init)
        self.lamda = torch.nn.parameter.Parameter(
            torch.tensor([100.0], requires_grad=True).cuda())
        print("lamda", self.lamda)
        self.infer_env_mean = torch.nn.parameter.Parameter(
            torch.tensor([0.5], requires_grad=True).cuda())
        ...
        self.optimizer_infer_env = torch.optim.Adam(self.params, lr=0.005)
        self.flags = flags
    ...
    def __call__(self, batch_data, step,  mlp=None, scale=None, **kwargs):
        ...
        normed_z = (train_z.float() - train_z.float().mean()) / \
            train_z.float().std()
        normed_x = (train_x.float() - train_x.float().mean()) / \
            train_x.float().std()
        ...
        train_loss = (infered_envs*self.relu(train_nll-self.eta) +
                      self.eta).mean()-cons_infered_envs*self.lamda
