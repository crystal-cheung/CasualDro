...
class InferIrmV1:
    ...
    def __call__(self, batch_data, step,  mlp=None, scale=None, **kwargs):
        ...
        if step < self.flags.penalty_anneal_iters:
            ...
            (-train_penalty).backward(retain_graph=True,inputs=list(self.infer_env.parameters()))
            wandb.log({"infered_envs": infered_envs.mean()})
            wandb.log({"max_loss": -train_penalty})
